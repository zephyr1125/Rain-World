...
Shader "Futile/Adrenaline" //Unlit Transparent Vertex Colored Additive 
{
Properties 
	{
...
half4 frag (v2f i) : COLOR
{
     ...
     half effect = lerp(-1, dot(dir, half2(lerp(-1, 1, i.clr.x), lerp(-1, 1, i.clr.y))), i.clr.z);
     
       float diff = pow(sin(dist*3.14), 30.0 + 28.0*effect)*0.015 * effect * i.clr.w;
     
     
     half2 grabPos = half2(i.scrPos.x, 1-i.scrPos.y) - (dir * diff);// * i.clr.w);
