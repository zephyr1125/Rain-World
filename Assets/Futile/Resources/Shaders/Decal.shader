...
Shader "Futile/Decal" //Unlit Transparent Vertex Colored Additive 
{
Properties 
	{
...
float ugh = fmod(fmod(   tex2D(_MainTex, float2(textCoord.x, textCoord.y)).x*255   , 90)-1, 30)/300.0;
...
if (texcol.x == 1.0 && texcol.y == 1.0 && texcol.z == 1.0){
  ...
  terrainCol = lerp(terrainCol, tex2D(_PalTex, float2(1.5/32.0, 7.5/8.0)), clamp(red*_fogAmount/30.0, 0, 1));
   ...
   if (red >= 5){
		float4 grabTexCol = tex2D(_GrabTexture, float2(i.scrPos.x, 1-i.scrPos.y));
