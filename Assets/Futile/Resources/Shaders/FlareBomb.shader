...
Shader "Futile/FlareBomb" //Unlit Transparent Vertex Colored Additive 
{
Properties 
	{
...
if(texcol.x == 1.0 && texcol.y == 1.0 && texcol.z == 1.0) dist = 1;
...
for (int j = 0; j < 4; j++) {
texcol = tex2D(_LevelTex, shadowPos - dir*0.02*j*centerDist);
half shadowDist = fmod((texcol.x * 255)-1, 30.0)/30.0;
if(texcol.x == 1.0 && texcol.y == 1.0 && texcol.z == 1.0) shadowDist = 1.0;


if (shadowDist > 6.0/30.0){
half4 grabColor = tex2D(_GrabTexture, oldShadowPos - dir*0.02*j*centerDist);
if( grabColor.x > 1.0/255.0 || grabColor.y != 0.0 || grabColor.z != 0.0) 
shadowDist = 6.0/30.0;
}

if(dist > shadowDist) //shadow = 1;
...
if(alpha < 0.2) alpha = 1.0-(alpha * 5.0);
else alpha = clamp((alpha - 0.2) * 1.3, 0, 1);

alpha = lerp(1.0-alpha, 1, pow(dist, 0.5));
alpha *= pow(pow((1-pow(centerDist * 2, 2)), 1.5), lerp(0.5, 2.5, dist));
...
alpha = pow(alpha, lerp(1, 0.2, pow(alpha, 0.4)));
//if(alpha > 0.3) alpha += pow(alpha-0.3, 0.1);

alpha = lerp(alpha, 1, pow(1-centerDist*2, 3) * i.clr.w);

//dist = pow(dist, 0.2);
if(alpha < 0.5)
 return half4(i.clr.xyz, alpha*2);
else
 return half4(lerp(i.clr.xyz, half3(1,1,1), (alpha-0.5)*2), 1);
