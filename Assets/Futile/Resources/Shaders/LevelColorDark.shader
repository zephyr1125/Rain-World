...
Shader "Futile/LevelColorDark" //Unlit Transparent Vertex Colored Additive 
{
	Properties 
	{
...
//if(texcol.y * 255 > 7 && texcol.y * 255 < 11) return float4(0,0,1,1);
  ...
  // setColor = tex2D(_PalTex, float2(0.5/32.0, 7.5/8));
   ...
   shadow = clamp(((shadow - 0.5)*6)+0.5-(_light*4), 0,1);//clamp((shadow - 0.6 - _light)*3, 0, 1);
 // return float4(1-shadow, 0, 0, 1);
   ...
   if (shadow != 1 && red > 5) {
   	float4 grabTexCol2 = tex2D(_GrabTexture, float2(screenPos.x + -_lightDirAndPixelSize.x*_lightDirAndPixelSize.z*(red-5)*1.2, 1-screenPos.y + -_lightDirAndPixelSize.y*_lightDirAndPixelSize.w*(red-5)*1.2));
    ...
    setColor = lerp(tex2D(_PalTex, float2(red/32.0, (paletteColor + 3 + 0.5)/8.0)), half4(0,0,0,1), shadow);


	half rbcol = (sin((_RAIN + (tex2D(_NoiseTex, float2(i.uv.x*2, i.uv.y*2) ).x * 4) + red/12.0) * 3.14 * 2)*0.5)+0.5;
  	setColor = lerp(setColor, tex2D(_PalTex, float2((5.5 + rbcol*25)/32.0, 6.5 / 8.0) ), (green > 3 ? 0.1 : 0.0)*(1-shadow));
  ...
  if(green > 3){
   ...
   if (green > 0 && green < 3){
    setColor = lerp(setColor, lerp(lerp(tex2D(_PalTex, float2(30.5/32.0, (5.5-(green-1)*2)/8.0)), half4(0,0,0,1), shadow), lerp(tex2D(_PalTex, float2(30.5/32.0, (4.5-(green-1)*2)/8.0)), tex2D(_PalTex, float2(31.5/32.0, (4.5-(green-1)*2)/8.0)), shadow), red/30.0), texcol.z);
  }else if (green == 3){
...
// setColor = lerp(setColor, half4(0, 0, 0, 1), red*0.5);
...
//    }else4
//    {
//    setColor = half4(0.3,0.3,0.3,0.5);
//    }
...
//setColor.x *= 0.5;
    ...
    return setColor;//half4(displace, displace, displace, 1);
