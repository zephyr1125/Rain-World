...
Shader "Futile/LocalBloom" //Unlit Transparent Vertex Colored Additive 
{
Properties 
	{
...
if(distance(i.uv, half2(0.5, 0.5)) > 0.5) return half4(0,0,0,0);
...
half2 screenPos = half2(i.scrPos.x, 1-i.scrPos.y);
    ...
    float coef=1.0;
    float fI = 0;
    float _BlurAmount = 0.0012;// * (0.5 + 0.5 * pow(i.clr.w * lerp(1, 0, distance(i.uv, half2(0.5, 0.5)) *2.0), 0.25));
    ...
    for (int j = 0; j < 3; j++) {
    	...
    	texcol = tex2D(_GrabTexture, float2(screenPos.x - fI * _BlurAmount * horFac, screenPos.y - fI * _BlurAmount)) * coef;
    	getCol += texcol/1.5;
     ...
     getCol /= div;
     getCol *= i.clr.w * lerp(1, 0, distance(i.uv, half2(0.5, 0.5)) *2.0);
