...
Shader "Futile/SceneOverlay" //Unlit Transparent Vertex Colored Additive 
{
Properties 
	{
...
//  getPos.y -= ((0.5 - i.scrPos.y) * dp*50) / spriteSize.y;
  ...
  if(grabCol.x < 0.5) r = getCol.x * (grabCol.x*2);
  else r = 1.0 - (1.0-getCol.x) * 2*(1.0-grabCol.x);
  
  if(grabCol.y < 0.5) g = getCol.y * (grabCol.y*2);
  else g = 1.0 - (1.0-getCol.y) * 2*(1.0-grabCol.y);
  
  if(grabCol.z < 0.5) b = getCol.z * (grabCol.z*2);
  else b = 1.0 - (1.0-getCol.z) * 2*(1.0-grabCol.z);
  
  
  return half4(r, g, b, getCol.w*i.clr.w);
