...
Shader "Futile/SpecificDepth" //Unlit Transparent Vertex Colored Additive 
{
Properties 
	{
...
half terrainDpth = (((int)(texcol.x * 255)-1) % 30)/30.0;
...
if(floor(dpth*30) == floor(terrainDpth*30))// dpth > terrainDpth-1.0/30.0 && dpth < terrainDpth + 1.0/30.0)
return half4(i.clr.xyz, tex2D(_MainTex, i.uv).w);
else
