...
Shader "Futile/SceneSoftLight" //Unlit Transparent Vertex Colored Additive 
{
Properties 
	{
...
half SoftLight(half hA, half hB)
{
if(hB < 0.5)
return 2 * hA * hB + hA*hA * (1.0 - 2 * hB);
else
return 2 * hA * (1.0 - hB) + sqrt(hA)*(2*hB - 1);
}
...
//  getPos.y -= ((0.5 - i.scrPos.y) * dp*50) / spriteSize.y;
  ...
  r = SoftLight(grabCol.x, getCol.x);
  g = SoftLight(grabCol.y, getCol.y);
  b = SoftLight(grabCol.z, getCol.z);
