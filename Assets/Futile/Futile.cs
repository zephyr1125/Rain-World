...
public class Futile : MonoBehaviour
{
    ...
    static public float displayScale { get { return 1f; } set { } } //set based on the resolution setting (the unit to pixel scale)
    static public float displayScaleInverse { get { return 1f; } set { } } // 1/displayScale
    ...
    public void Init(FutileParams futileParams)
    {
        ...
        _camera.depth = 100;
        _camera.rect = new Rect(0.0f, splitScreen ? 0.5f : 0f, 1f, 1f);
        ...
        if (splitScreen) {
            ...
            _cameraHolder2.transform.parent = gameObject.transform;
            ...
            _camera2.nearClipPlane = 0.0f;
            _camera2.farClipPlane = 500.0f;
            _camera2.depth = 100;
            _camera2.rect = new Rect(0.0f, 0f, 1.0f, 0.5f);
            _camera2.backgroundColor = _futileParams.backgroundColor;

            //we multiply this stuff by scaleInverse to make sure everything is in points, not pixels
            _camera2.orthographic = true;
            _camera2.orthographicSize = (screen.pixelHeight / 2 * displayScaleInverse) * 0.5f;
        }
    ...
    {
        _camera.orthographicSize = screen.pixelHeight / 2 * displayScaleInverse * (splitScreen ? 0.5f : 1f);
        ...
        if (splitScreen) {
            ...
            camXOffset = ((screen.originX - 0.5f) * -screen.pixelWidth) * displayScaleInverse + screenPixelOffset - 6000f;
            camYOffset = ((screen.originY - 0.5f) * -screen.pixelHeight) * displayScaleInverse - screenPixelOffset;

            _camera2.transform.position = new Vector3(camXOffset, camYOffset, -10.0f);
        }
